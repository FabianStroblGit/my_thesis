...
class CognitiveMapNetwork:
    ...
    def __init__(self, dt, from_data=False):
        ...
        # Index of the PC that was active in the previous time step. None on
        # the very first call so that the topology-update path skips creating
        # connections from a phantom prior PC.
    ...
    def track_movement(self, pc_firing, created_new_pc, reward, env=None, current_step=0,
        ...
        Args:
            pc_network: PlaceCellNetwork, if provided, new connections are only formed
                between PCs whose env_coordinates are within max_connection_dist.
            max_connection_dist: float, maximum distance (meters) between PCs for a
                connection to be created. Prevents long-distance shortcuts through walls.
        """
        ...
        if pc_active > self.active_threshold:
            ...
            # Update visit tracking for A2C intrinsic reward
        ...
        # (skip on first visit when prior_pc is None).
        if entered_different_pc and prior_pc is not None:
            # Ensure the prior PC has recency above threshold so a connection
            # can form. Handles the case where the agent comes from an
            # existing PC (loaded from data) whose recency was initialised to 0.
            ...
            # Long-distance connections often pass through walls and create
            # false shortcuts in the topology graph.
            ...
            # save bilateral connections; track whether topology actually
            # changed so we can skip the expensive 100-iteration reward
            # propagation when no new edge was added.
            ...
            # update reward cells, refer to thesis for formulas — only when
            # the topology graph actually gained new edges. PC transitions
            # within an unchanged graph reproduce the same reward cells; the
            # propagation is wasted work. With dense PCs near the goal the
            # agent flickers between cells repeatedly, so skipping no-op
            # propagations is the dominant per-step cost saving.
            if topology_changed:
        ...
        # Update prior PC (always, even on first visit).
    ...
    def path_exists(self, start_pc, goal_pc):
        ...
        Args:
            start_pc: int, start place cell index
            goal_pc: int, goal place cell index
            
        Returns:
            bool: True if a path exists through the topology graph
        """
    ...
    def find_shortest_path(self, start_pc, goal_pc):
        ...
        of place cells to traverse — analogous to how replay events in the
        hippocampus activate place cell sequences for route planning.
        ...
        Returns:
            list of int: sequence of PC indices from start to goal (inclusive),
    ...
    def set_goal_pc_by_location(self, goal_location, pc_network):
        ...
        Returns the index of the goal PC."""
        best_idx = None
        best_dist = float('inf')
        ...
        if best_idx is not None:
            # Reset all rewards, only the true goal PC gets 1.0
    ...
    def prune_blocked_connections(self, current_pc, agent_pos, directions, pc_network):
        ...
        Biologically plausible: if the agent is at a place cell and perceives
        that the direction toward a connected neighbor is blocked (wall detected
        by whiskers/raycasts), the connection is pruned from the cognitive map.
        This prevents reward from propagating through walls.
        
        Safety: never prunes a connection that would disconnect the goal PC
        from the rest of the graph.
        
        Args:
            current_pc: int, index of the place cell the agent is currently at
            agent_pos: np.array([x, y]), agent's current position
            directions: np.array of bool, 16 ray directions (True=free, False=blocked)
            pc_network: PlaceCellNetwork, to access neighbor coordinates
        
        Returns:
            bool: True if any connections were pruned
        """
        ...
        for nb in neighbors:
            ...
            # Compute angle from agent to neighbor PC
            delta = nb_coords - agent_pos
            angle_to_nb = np.arctan2(delta[1], delta[0])
            if angle_to_nb < 0:
                angle_to_nb += 2 * np.pi
            
            # Find nearest ray direction index
            ...
            # is definitely behind a wall — prune the connection
            if (not directions[nearest_ray_idx] 
                    and not directions[left_idx] 
                    and not directions[right_idx]):
                dist_to_nb = np.linalg.norm(delta)
                # Only prune if neighbor is close enough that rays would detect the wall
                # (ray length is ~2m; if neighbor is 5m away, rays don't tell us about walls there)
                if dist_to_nb < 3.0:
                    # SAFETY: don't prune if it would disconnect the goal
                    if goal_pc is not None:
                        # Temporarily remove the connection and check path
                        self.topology_cells[current_pc][nb] = 0
                        self.topology_cells[nb][current_pc] = 0
                        # Check if goal is still reachable from BOTH sides
                        still_connected = self.path_exists(current_pc, goal_pc) or self.path_exists(nb, goal_pc)
                        if not still_connected:
                            # Restore — this connection is critical
                            self.topology_cells[current_pc][nb] = 1
                            self.topology_cells[nb][current_pc] = 1
                            continue
                        # Also check that nb can still reach the goal (don't isolate it)
                        if not self.path_exists(nb, goal_pc):
                            # nb would be cut off — only prune if nb is on a dead-end
                            # (no other path from nb to goal)
                            self.topology_cells[current_pc][nb] = 1
                            self.topology_cells[nb][current_pc] = 1
                            continue
                    else:
