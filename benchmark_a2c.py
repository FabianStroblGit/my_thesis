...
PLOT_DIR = ROOT / "benchmark_a2c_plots"
...
def main():
    parser = argparse.ArgumentParser(description="A2C benchmark with multiple runs")
    ...
    parser.add_argument("--pretrained", type=Path, default=None,
                        help="Path to a pre-trained A2C checkpoint. When provided, "
                             "each evaluation run loads this checkpoint and runs with "
                             "a frozen policy (no learning during eval). This produces "
                             "the standard 'train then test' A2C protocol, making the "
                             "comparison to zero-shot Active Inference fair.")
    ...
    parser.add_argument("--legacy-thigmo", action="store_true",
                        help="Use the thigmotaxis stack as it stood at commit "
                             "4844e77: 9-ray goal-direction fan with no "
                             "walls-required / wall-perpendicular guards, no "
                             "rejection-streak timeout, no cooldown gate, no "
                             "nav-step frustration gate, and unchecked "
                             "retreat-to-safe-position teleports. Useful for "
                             "A/B comparison against the current thigmotaxis "
                             "implementation.")
    args = parser.parse_args()
    n_runs = args.runs
    pretrained_path = args.pretrained.resolve() if args.pretrained else None
    live = args.live
    if args.thigmo == "on":
    ...
    if pretrained_path is not None and not pretrained_path.exists():
        sys.exit(f"[benchmark] --pretrained checkpoint not found: {pretrained_path}")
    ...
    print("\n" + "="*70)
    print(f"  A2C BENCHMARK  —  init + 2 configs x {n_runs} runs  (150k step limit)")
    ...
    for door_idx in door_list:
        ...
        for thigmo in thigmo_settings:
            ...
            config_label = f"A2C / door={door_idx} / thigmotaxis={thigmo_str}"
            print(f"\n{'='*60}")
            ...
            for run_idx in range(n_runs):
                ...
                plot_file = plot_dir_run / (
                    f"a2c_door{door_idx}_thigmo{thigmo_str}_run{run_idx+1:02d}.png"
                )
