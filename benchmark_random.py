...
PLOT_DIR = ROOT / "benchmark_random_plots"
...
def summarize(runs: list, metric_key: str) -> str:
...
def main():
    parser = argparse.ArgumentParser(description="Random-walk benchmark with multiple runs")
    ...
    parser.add_argument("--doors", type=str, default="1",
                        help="Comma-separated list of door indices to evaluate (1..5). "
                             "Default '1'; pass '1,2,3,4,5' for full coverage.")
    parser.add_argument("--thigmo", choices=["on", "off", "both"], default="both",
                        help="Which thigmotaxis settings to evaluate.")
    parser.add_argument("--live", action="store_true",
                        help="Show the live cognitive-map matplotlib window during each run.")
    parser.add_argument("--seed-base", type=int, default=None,
                        help="If set, each run uses rng_seed = seed_base + run_index "
                             "for reproducibility. Default: fresh seed per run.")
    ...
    print("\n" + "=" * 70)
    print(f"  RANDOM-WALK BENCHMARK  —  init + N configs x {n_runs} runs  (150k step limit)")
    ...
    for door_idx in door_list:
        ...
        for thigmo in thigmo_settings:
            ...
            config_label = f"random / door={door_idx} / thigmotaxis={thigmo_str}"
            print(f"\n{'='*60}")
            ...
            for run_idx in range(n_runs):
                ...
                plot_file = plot_dir_run / (
                    f"random_door{door_idx}_thigmo{thigmo_str}_run{run_idx+1:02d}.png"
                )

                rnd_override = {}
                if args.seed_base is not None:
                    rnd_override["rng_seed"] = args.seed_base + run_idx
                ...
                metrics = run_with_config(
                    ...
                    cfg_override={
                        ...
                        "random_exploration": rnd_override,
